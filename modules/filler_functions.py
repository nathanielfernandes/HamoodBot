...
class Filler():
    def __init__(self, size, playerOne, playerTwo, msg):
    ...
    def fill_board(self):
        ...
        self.grid[18] = self.grid[18] if self.grid[71] != self.grid[18] else (self.grid[18] + 1) if self.grid[18] + 1 <= 5 else self.grid[18] - 1
    ...
    def get_winner(self):
        winner = self.playerOne if self.amountOne > self.amountTwo else self.playerTwo if self.amountOne != self.amountTwo else False
        return winner

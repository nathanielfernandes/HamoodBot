...
class Soko_ban:
    ...
    def __init__(self, size, user, server):
    ...
    def create_level(self):
        ...
        for pos in self.box_pos:
            if (
                (pos in self.box_pos)
                and (pos + 1 in self.box_pos)
                and (pos + self.size[0] in self.box_pos)
                and (pos + self.size[0] + 1 in self.box_pos)
            ):
                self.create_level()
            ...
            if self.grid[self.new_player_pos] in [2, 3]:
                ...
                if self.move == "up":
                    if self.grid[self.new_player_pos - self.size[0]] not in [1, 2, 3]:
                        ...
                        self.box_pos[(self.box_pos).index(self.new_player_pos)] = (
                            self.new_player_pos - self.size[0]
                        )
                    else:
                        return
